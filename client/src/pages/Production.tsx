...
export default function Production() {
  ...
  return <StudioShell title="Produção" eyebrow="Régua de execução" actions={<NewProjectLink />}>
    <div className="ops-content">
      ...
      <section className="ops-two-column ops-production-lower"><div className="ops-panel"><div className="ops-panel-heading"><div><p className="ops-section-kicker">Agenda operacional</p><h2>Próximos marcos</h2></div><CircleDotDashed size={20} /></div>{agenda.data?.length ? <div className="ops-agenda-list">{agenda.data.slice(0, 8).map(({ event, project, responsible }) => <div key={event.id} className="ops-agenda-item"><time>{formatDate(event.startsAt, true)}</time><span><strong>{event.title}</strong><small>{project?.name ?? "Evento da operação"}{responsible ? ` · ${responsible.name}` : ""}</small></span></div>)}</div> : <div className="ops-empty"><p>Não há marcos na agenda. Registre reuniões, entregas e revisões para torná-los visíveis à equipe.</p></div>}</div><IntegrationPanel connectionUrl={connectionUrl} setConnectionUrl={setConnectionUrl} connection={connection.data} integration={integration.data} loading={integration.isLoading} projects={projects.data ?? []} projectToAttach={projectToAttach} setProjectToAttach={setProjectToAttach} selectedFiles={selectedFiles} toggleFile={toggleFile} onConnect={() => connect.mutate({ baseUrl: connectionUrl })} connecting={connect.isPending} onAttach={() => projectToAttach !== "no-project" && attach.mutate({ projectId: Number(projectToAttach), fileNames: selectedFiles })} attaching={attach.isPending} /></section>
    </div>
